...
class DataArray3D : public DataArray {
   public:
    ...
    DataArray3D(
        ...
        M = std::shared_ptr<array_type>(
            new array_type(boost::extents[xdim][ydim][zdim], so));
    }
    DataArray3D(std::unique_ptr<char[]>& data, unsigned int xdim,
                unsigned int ydim, unsigned int zdim)
        : DataArray3D(xdim, ydim, zdim) {
        std::memcpy(M->origin(), data.get(), xdim * ydim * zdim * sizeof(T));
    }
    ~DataArray3D() { M.reset(); }

    array_view view(const std::array<int, 2>& xrng,
                    const std::array<int, 2>& yrng,
                    const std::array<int, 2>& zrng) const {
        return (*M)[boost::indices[range(xrng[0], xrng[1])][range(
            yrng[0], yrng[1])][range(zrng[0], zrng[1])]];
    }
    ...
    void copy(std::unique_ptr<char[]>& data, unsigned int xdim,
              unsigned int ydim, unsigned int zdim) {
