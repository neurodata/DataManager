...
void BloscArray32::load(const std::string& filename) {
    ...
    for(size_t x=0; x<shape[0]; x++) {
        for(size_t y=0; y<shape[1]; y++) {
            for(size_t z=0; z<shape[2]; z++) {
                ...
                (*M)[static_cast<index>(x)][static_cast<index>(y)][static_cast<index>(z)] = static_cast<uint32_t>(uncompressed[i]);
            }
        }
    }
}

void BloscArray32::save(const std::string& filename) {
