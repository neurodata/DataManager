...
Scale::Scale(const folly::dynamic& obj) {
    ...
    CHECK(sizeMember != obj.items().end() && sizeMember->second.isArray() &&
          sizeMember->second.size() == 3)
    ...
    CHECK(voxelOffsetMember != obj.items().end() &&
          voxelOffsetMember->second.isArray() &&
    ...
    CHECK(resolutionMember != obj.items().end() &&
          resolutionMember->second.isArray() &&
    ...
    CHECK(chunkSizesMember != obj.items().end() &&
          chunkSizesMember->second.isArray())
    ...
    for (const auto& chunkSizeObj : chunkSizesMember->second) {
        ...
        chunk_sizes.push_back(
            std::array<int, 3>({{static_cast<int>(chunkSizeObj[0].asInt()),
                                 static_cast<int>(chunkSizeObj[1].asInt()),
    ...
    CHECK(encodingMember != obj.items().end() &&
          encodingMember->second.isString())
        << "Error: Scale property 'encoding' is required and it must be a "
           "string.";
    encoding = encodingMember->second.asString();

    auto compressedSegmentationBlockSizeMember =
        obj.find("compressed_segmentation_block_size");
    if (compressedSegmentationBlockSizeMember != obj.items().end()) {
        CHECK(compressedSegmentationBlockSizeMember->second.isArray())
            << "Error: Scale property compressed_segmentation_block_size must "
               "be an array.";
        for (int i = 0; i < 3; i++) {
            compressed_segmentation_block_size[i] =
                compressedSegmentationBlockSizeMember->second[i].asInt();
        }
    }
}
...
folly::dynamic Scale::toDynamicObj() const {
    ...
    folly::dynamic voxelOffsetObj = folly::dynamic::array(
        voxel_offset[0], voxel_offset[1], voxel_offset[2]);

    folly::dynamic resolutionObj =
        folly::dynamic::array(resolution[0], resolution[1], resolution[2]);

    folly::dynamic chunkSizesObj = folly::dynamic::array();
    for (const auto& chunk : chunk_sizes) {
        chunkSizesObj.push_back(
            folly::dynamic::array(chunk[0], chunk[1], chunk[2]));
    }

    folly::dynamic compressedSegmentationBlockSizeObj =
        folly::dynamic::array(compressed_segmentation_block_size[0],
                              compressed_segmentation_block_size[1],
                              compressed_segmentation_block_size[2]);

    folly::dynamic jsonObj = folly::dynamic::object("key", key)(
        "size", sizeObj)("voxel_offset", voxelOffsetObj)(
        "resolution", resolutionObj)("chunk_sizes", chunkSizesObj)(
        "encoding", encoding)("compressed_segmentation_block_size",
                              compressedSegmentationBlockSizeObj);
...
void Manifest::Write(const std::string& filename) {
    ...
    folly::dynamic jsonObj =
        folly::dynamic::object("type", _type)("data_type", _data_type)(
    ...
    try {
        ...
        LOG(FATAL) << "Error: Failed to write Neuroglancer manifest to disk. "
                   << ex.what();
    }
}
...
void Manifest::Read(const std::string& filename) {
    ...
    std::string jsonObjStr(static_cast<std::stringstream const&>(
                               std::stringstream() << ifs.rdbuf())
                               .str());
    ...
    CHECK(typeMember != parsed.items().end())
        << "Error: Type is a required field in the Neuroglancer Manifest.";
    ...
    CHECK(numChannelsMember != parsed.items().end())
        << "Error: Num Channels is a required field in the Neuroglancer "
           "Manifest.";
    set_num_channels(numChannelsMember->second.asInt());

    auto scalesMember = parsed.find("scales");
    CHECK(scalesMember != parsed.items().end() &&
          scalesMember->second.isArray())
