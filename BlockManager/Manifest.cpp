...
Scale::Scale(const folly::dynamic& obj) {
    // This constructor assumes that obj has already been validated to be an array
    auto keyMember = obj.find("key");
    CHECK(keyMember != obj.items().end() && keyMember->second.isString()) << "Error: Scale property 'key' is required and it must be a string.";
    key = keyMember->second.asString();

    auto sizeMember = obj.find("size");
    CHECK(sizeMember != obj.items().end() && sizeMember->second.isArray() && sizeMember->second.size() == 3) << "Error: Scale property 'size' is required and it must be an array with 3 elements.";
    for(int i=0; i<3; i++) {
        size[i] = sizeMember->second[i].asInt();
    }

    auto voxelOffsetMember = obj.find("voxel_offset");
    CHECK(voxelOffsetMember != obj.items().end() && voxelOffsetMember->second.isArray() && voxelOffsetMember->second.size() == 3) << "Error: Scale property 'voxel_offset' is required and it must be an array with 3 elements.";
    for(int i=0; i<3; i++) {
        voxel_offset[i] = voxelOffsetMember->second[i].asInt();
    }

    auto resolutionMember = obj.find("resolution");
    CHECK(resolutionMember != obj.items().end() && resolutionMember->second.isArray() && resolutionMember->second.size() == 3) << "Error: Scale property 'resolution' is required and it must be an array with 3 elements.";
    for(int i=0; i<3; i++) {
        resolution[i] = resolutionMember->second[i].asDouble();
    }

    auto chunkSizesMember = obj.find("chunk_sizes");
    CHECK(chunkSizesMember != obj.items().end() && chunkSizesMember->second.isArray()) << "Error: Scale property 'chunk_sizes' is required and it must be an array.";
    for(const auto& chunkSizeObj : chunkSizesMember->second) {
        CHECK(chunkSizeObj.isArray() && chunkSizeObj.size() == 3) << "Error: All 'chunk_size' elements must be arrays of size 3.";
        chunk_sizes.push_back(std::array< int, 3 >({{static_cast<int>(chunkSizeObj[0].asInt()), static_cast<int>(chunkSizeObj[1].asInt()), static_cast<int>(chunkSizeObj[2].asInt())}}));
    }

    auto encodingMember = obj.find("encoding");
    // TODO(adb): validate encoding string against list of supported encodings
    CHECK(encodingMember != obj.items().end() && encodingMember->second.isString()) << "Error: Scale property 'encoding' is required and it must be a string.";
    ...
    if (compressedSegmentationBlockSizeMember != obj.items().end()) {
        CHECK(compressedSegmentationBlockSizeMember->second.isArray()) << "Error: Scale property compressed_segmentation_block_size must be an array.";
        for(int i=0; i<3; i++) {
...
folly::dynamic Scale::toDynamicObj() const {
    ...
    folly::dynamic compressedSegmentationBlockSizeObj = folly::dynamic::array(compressed_segmentation_block_size[0], compressed_segmentation_block_size[1], compressed_segmentation_block_size[2]);

    folly::dynamic jsonObj = folly::dynamic::object("key", key)("size", sizeObj)("voxel_offset", voxelOffsetObj)("resolution", resolutionObj)("chunk_sizes", chunkSizesObj)("encoding", encoding)("compressed_segmentation_block_size", compressedSegmentationBlockSizeObj);
...
void Manifest::Write(const std::string& filename) {
    ...
    folly::dynamic jsonObj = folly::dynamic::object("type", _type)("data_type", _data_type)("num_channels", _num_channels)("scales", scaleArrObj);
...
void Manifest::Read(const std::string& filename) {
    ...
    CHECK(dataTypeMember != parsed.items().end()) << "Error: Data Type is a required field in the Neuroglancer Manifest.";
    set_data_type(dataTypeMember->second.asString());

    auto numChannelsMember = parsed.find("num_channels");
    CHECK(numChannelsMember != parsed.items().end()) << "Error: Num Channels is a required field in the Neuroglancer Manifest.";
    set_num_channels(numChannelsMember->second.asInt());

    auto scalesMember = parsed.find("scales");
    CHECK(scalesMember != parsed.items().end() && scalesMember->second.isArray()) << "Error: Scales is a required field in the Neuroglancer Manifest and it must be an array.";
