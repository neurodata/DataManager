...
BlockManager::BlockManager(const std::string &directory_path_name,
                           std::shared_ptr<Manifest> manifestShPtr,
                           const bool use_gzip)
    : directory_path_name(directory_path_name), manifest(manifestShPtr),
      _use_gzip(use_gzip) {
  _init();
}
...
std::array<int, 3>
BlockManager::getChunkSizeForScale(const std::string &scale_key) {
...
std::array<int, 3>
BlockManager::getVoxelOffsetForScale(const std::string &scale_key) {
  const auto scale = manifest->get_scale(scale_key);
  return std::array<int, 3>(
      {{scale.voxel_offset[0], scale.voxel_offset[1], scale.voxel_offset[2]}});
}
...
std::vector<BlockKey> BlockManager::_blocksForBoundingBox(
  ...
  _xrng[0] =
      floor(static_cast<double>(xrng[0]) / static_cast<double>(chunk_size[0]));
  _xrng[1] =
      ceil(static_cast<double>(xrng[1]) / static_cast<double>(chunk_size[0]));

  _yrng[0] =
      floor(static_cast<double>(yrng[0]) / static_cast<double>(chunk_size[1]));
  _yrng[1] =
      ceil(static_cast<double>(yrng[1]) / static_cast<double>(chunk_size[1]));

  _zrng[0] =
      floor(static_cast<double>(zrng[0]) / static_cast<double>(chunk_size[2]));
  _zrng[1] =
      ceil(static_cast<double>(zrng[1]) / static_cast<double>(chunk_size[2]));

  std::vector<BlockKey> ret;
  ret.reserve((_xrng[1] - _xrng[0] + 1) * (_yrng[1] - _yrng[0] + 1) *
              (_zrng[1] - _zrng[0] + 1));
  for (int x = _xrng[0]; x < _xrng[1]; x++) {
    for (int y = _yrng[0]; y < _yrng[1]; y++) {
      for (int z = _zrng[0]; z < _zrng[1]; z++) {
        uint64_t morton_idx =
            Morton64::XYZMorton(std::array<int, 3>({{x, y, z}}));
...
void BlockManager::_init() {
  ...
  for (const auto &scale : manifest->_scales) {
    std::shared_ptr<BlockMortonIndexMap> blockIdxMap =
        _createIndexForScale(scale.key);
...
BlockManager::_createIndexForScale(const std::string &scale) {
  std::shared_ptr<BlockMortonIndexMap> ret =
      std::make_shared<BlockMortonIndexMap>();

  // For each scale, iterate through the files in that directory
  const auto path = fs::path(directory_path_name);
  CHECK(fs::is_directory(path / fs::path(scale)))
      << "Error: Directory path for scale " << scale << " is not a directory!";
