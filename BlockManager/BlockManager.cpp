...
std::array<int, 3> BlockManager::getChunkSizeForScale(
...
std::array<int, 3> BlockManager::getVoxelOffsetForScale(
    const std::string& scale_key) {
    const auto scale = manifest->get_scale(scale_key);
    return std::array<int, 3>({{scale.voxel_offset[0], scale.voxel_offset[1],
...
std::vector<BlockKey> BlockManager::_blocksForBoundingBox(
    ...
    _xrng[0] = floor(static_cast<double>(xrng[0]) /
                     static_cast<double>(chunk_size[0]));
    _xrng[1] =
        ceil(static_cast<double>(xrng[1]) / static_cast<double>(chunk_size[0]));

    _yrng[0] = floor(static_cast<double>(yrng[0]) /
                     static_cast<double>(chunk_size[1]));
    _yrng[1] =
        ceil(static_cast<double>(yrng[1]) / static_cast<double>(chunk_size[1]));

    _zrng[0] = floor(static_cast<double>(zrng[0]) /
                     static_cast<double>(chunk_size[2]));
    _zrng[1] =
...
std::shared_ptr<BlockMortonIndexMap> BlockManager::_createIndexForScale(
    ...
    CHECK(fs::is_directory(path / fs::path(scale)))
        << "Error: Directory path for scale " << scale
        << " is not a directory!";
