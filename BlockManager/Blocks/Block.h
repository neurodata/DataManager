...
public:
  ...
  Block(const std::string &path_name, int xdim, int ydim, int zdim,
        size_t dtype_size, bool gzip = true)
      : path_name(path_name), _xdim(xdim), _ydim(ydim), _zdim(zdim),
        _dtype_size(dtype_size), _gzip(gzip) {}
  ~Block() {}
  ...
  template <typename T>
  void add(const typename DataArray_namespace::DataArray3D<T>::array_view &view,
...
protected:
  ...
  bool _gzip; // TODO(adb): tmp var, will remove in block refactor
