...
// TODO(adb): we should probably move this to Block.h... in fact we could probably collapse AnnoBlock and ImageBlock into a generic templated Block and then subclass the save / mode methods based on backend
...
class AnnoBlock32 : public Block {
   public:
    ...
    AnnoBlock32(const std::string& path_name, int xdim, int ydim, int zdim,
                size_t dtype_size, const std::string& encoding)
        : Block(path_name, xdim, ydim, zdim, dtype_size) {
